...
public class ParseDriver {
     ...
     * @return parsed AST
     * <p>
     * TODO Antlr对Hive SQL解析的代码如如下方法的逻辑，HiveLexerX，HiveParser分别是 Antlr对语法文件 HiveLexer.g
     * TODO 编译后自动生成的词法解析和语法解析类，在这两个类中进行复杂的解析。
    ...
    public ASTNode parse(String command, Context ctx, String viewFullyQualifiedName) throws ParseException {
         ...
         *
         * TODO Antlr对语法文件 HiveLexer.g 编译后自动生成的词法解析和语法解析类（HiveLexerX，HiveParser）
         *  文件 HiveLexer.g 定义了一些 hive 的关键字，form、where，数字的定义格式【0–9】，分隔符，比较符之类的。
         *  每一个关键字分支都会变成一个 token。
         *
         * TODO HiveLexerX 是 antlr 根据词法规则文件，通过编译生成的一个代码类
        ...
        /**
         * TODO 根据词法分析的结果得到tokens的，此时不只是单纯的字符串，
         *  而是具有特殊意义的字符串的封装，其本身是一个流。
         *  lexer 把 SQL 语句中的各个语法分支，都转换成底层引擎能识别的各种 Token
         *
         *  SQL 使用词法和语法解析，来对SQL中的关键字 和 逻辑执行 TOK 替代
         */
        ...
        /**
         * TODO 语法解析 HiveParser是 Antlr 根据 HiveParser.g 生成的文件
         */
        ...
        try {
            /**
             * TODO 111 转化为 ASTTree 放在 ASTNode 中的 tree 属性中。 通过 r.getTree() 获取返回。
