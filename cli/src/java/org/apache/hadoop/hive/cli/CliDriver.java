...
public class CliDriver {
    ...
    /**
     * TODO 处理一个独立的 HQL 的解析和执行工作。
     * exit;   quit;
     * source /opt/sql/xxx.sql
     * select ... form ...
     * ...
     */
    public int processCmd(String cmd) {
        ...
        if (cmd_trimmed.toLowerCase().equals("quit") || cmd_trimmed.toLowerCase().equals("exit")) {
            ...
            // TODO_MA 注释: hive是支持在 hive交互界面去执行 HDFS 和一些 shell 操作命令的
            ...
            try {
                 ...
                 * TODO_MA 核心操作
                 *    获取处理器
                 *    processLocalCmd方法的逻辑：proc处理cmd
                 */
                CommandProcessor proc = CommandProcessorFactory.get(tokens, (HiveConf) conf);
                ret = processLocalCmd(cmd, proc, ss);

            } catch (SQLException e) {
                console.printError("Failed processing command " + tokens[0] + " " + e.getLocalizedMessage(), org.apache.hadoop.util.StringUtils
     ...
     * @return
     *
     * 这段代码最大的作用，就是帮你不停的重试去执行某个操作，直到达到最大重试次数
     */
                ...
                if (proc != null) {
                    ...
                    if (proc instanceof Driver) {
                        ...
                        try {
                            ...
                            console.printError("Failed with exception " + e.getClass().getName() + ":" + e
                        ...
                        /**
                         * TODO 44444 每个 HQL 语句执行完成之后，有一个 执行耗时的提示，就是在这儿执行的。
                         */
    ...
    public int processLine(String line, boolean allowInterrupting) {
        ...
        try {
            ...
            // we can not use "split" function directlyine as ";" may be quoted
            ...
            for (String oneCmd : commands) {
                ...
                /********
                 * TODO_MA 马中华 百万大数据
                 *   注释： 处理一个 独立的HQL 的执行command 就是一个能独立执行的SQL
                 */
        ...
        if (ss.initFiles.size() == 0) {
            if (System.getenv("HIVE_HOME") != null) {
                ...
                if (new File(hivercDefault).exists()) {
                    ...
                    console.printError("Putting the global hiverc in " + "$HIVE_HOME/bin/.hiverc is deprecated. " + "Please " + "use $HIVE_CONF_DIR"
                            + "/.hiverc instead.");
                }
            }
    ...
    public void processSelectDatabase(CliSessionState ss) throws IOException {
        ...
        if (database != null) {
            ...
            /**
             * TODO_MA 发现有意思的东西: processLine 才是执行一个 SQL 语句的方法实现
             */
    ...
    public int run(String[] args) throws Exception {
        ...
        /********
         * TODO_MA 马中华 百万大数据
         *   注释：初始化输入和输出流，负责接收SQL和打印执行结果
         *   SessionState: 管理会话的
         *   hive ===> hive > select ....
         *
         *   因为需要接收用户输入的sql,内部执行之后，打印结果输出，所以，必然会涉及到 System.in 和 System.out
         */
    ...
    private int executeDriver(CliSessionState ss, HiveConf conf, OptionsProcessor oproc) throws Exception {
        ...
        // TODO_MA 注释: 如果是 SQL 脚本文件  hive -f ""
        try {
            if (ss.fileName != null) {
        ...
        // TODO_MA 注释: 初始化从 Console 读取用户编辑的 SQL 命令等
    ...
    private void setupCmdHistory() {
        ...
        try {
            if ((new File(historyDirectory)).exists()) {
                ...
                System.err
                        .println("WARNING: Directory for Hive history file: " + historyDirectory + " does not " + "exist.   History will not be " + "available during this session.");
            }
        } catch (Exception e) {
            System.err
                    .println("WARNING: Encountered an error while trying to initialize Hive's " + "history " + "file.  History will not be " +
